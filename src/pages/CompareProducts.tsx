          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <Button asChild variant="ghost" size="sm" className="mb-2 text-muted-foreground">
                <Link to="/products"><ArrowLeft className="mr-1 h-3.5 w-3.5" /> Back to Store</Link>
              </Button>
              <h1 className="text-display-3 text-foreground">
          ...
          <table className="w-full min-w-[600px]">
            ...
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-4 pr-4 w-[200px] text-xs uppercase tracking-[0.15em] text-muted-foreground align-bottom">
                  Product
                </th>
                {items.map((product) => (
                  <th key={product.id} className="py-4 px-4 text-left align-bottom">
                    <div className="flex flex-col gap-2">
                      ...
                      <Link to={`/products/${product.slug}`} className="text-foreground font-semibold hover:text-accent transition-colors text-sm leading-tight">
                        {product.name}
                      </Link>
                      {product.badge && (
