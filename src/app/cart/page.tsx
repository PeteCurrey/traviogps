...
export default function CartPage() {
  ...
  usePageMeta(
    "Your Cart | Travio GPS Tracker Store",
    "Review your GPS tracker order. Free UK delivery on all orders with 2-year warranty and 30-day returns."
  );
                  ...
                  {items.map((item) => (
                    <div key={item.product.id} className="flex gap-4 md:gap-6 p-4 md:p-6 bg-card rounded-sm border border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link href={`/products/${item.product.slug}`} className="font-serif text-base md:text-lg text-foreground hover:text-accent transition-colors">
                          {item.product.name}
                        </Link>
