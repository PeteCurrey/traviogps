...
export default function ProductDetailPage() {
  ...
  usePageMeta(
    product ? `${product.name} — Buy Online | Travio GPS Tracker Store` : "Product Not Found",
    product ? `${product.shortDescription}. From £${product.price.toFixed(2)} with free UK delivery. ${product.rating}★ rated (${product.reviews.toLocaleString()} reviews).` : "",
  );
          ...
          <div className="container-premium">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {frequentlyBoughtTogether.map((p) => (
                <Link key={p.id} href={`/products/${p.slug}`} className="group block bg-card rounded-sm border border-border overflow-hidden hover:border-accent/50 transition-all">
      ...
      {related.length > 0 && (
        <section className="section-padding bg-card">
          <div className="container-premium">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((p) => (
                <Link key={p.id} href={`/products/${p.slug}`} className="group block bg-background rounded-sm border border-border overflow-hidden hover:border-accent/50 transition-all">
                  <div className="aspect-[4/3] bg-secondary/20 flex items-center justify-center">
                    <p.icon className="w-12 h-12 text-accent/40 group-hover:text-accent/70 transition-colors" />
                  </div>
                  <div className="p-4">
